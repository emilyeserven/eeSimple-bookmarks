...
## Media taxonomy family (Plex/Kavita-backed)

Eight sibling slug-routed taxonomies under `/taxonomies/`: **Media Properties** (a flat franchise/IP
grouping, no external link) and seven entities that optionally belong to one — **Movies, TV Shows,
Episodes, Artists, Albums, Tracks** (**Plex-backed**) and **Books** (**Kavita-backed**). The
**`media-taxonomy-family`** skill is the map of this subsystem — read it before touching any
`Plex*`/`*TaxonomyImage*` file or `services/movies.ts`/`services/books.ts` (and their five/zero
siblings). Two conventions worth knowing before you're in the skill:
- The six Plex-backed entities share **one generic edit/view/create form** (`PlexTitleGeneralForm`/
  `PlexTitleGeneralView`/`PlexTitleForm`, typed over a structural `PlexTitle` interface) — a field
  added there covers all six at once. Books aren't part of that abstraction and are hand-rolled.
- Every entity has a multi-image gallery (up to 12 images, one "main") backed by a **single shared
  polymorphic `taxonomy_images` table** (`services/taxonomyImages.ts` + `routes/taxonomyImageRoutes.ts`
  + client `TaxonomyImageGallery.tsx`), keyed by `(ownerType, ownerId)` — not seven duplicated
  bookmark_images-style tables. Its one gotcha: the byte-serving route is registered **once** globally
  (`registerTaxonomyImageServingRoute` in `app.ts`), never per entity.

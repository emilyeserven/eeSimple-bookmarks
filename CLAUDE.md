...
Migrations live in `packages/middleware/drizzle/`. On boot the gateway runs them
(`dist/db/migrate.js`) and then `drizzle-kit push --force` as a drift safety net. Production
databases were originally provisioned with `drizzle-kit push`, which stamps schema objects
**without** writing journal rows — so a migration can run against a database that already has the
column/constraint it adds. Generated migrations must therefore be **idempotent**: after
`pnpm --filter=@eesimple/middleware generate`, hand-edit the new `*.sql` (leave the `meta/` snapshots
as generated) so re-applying is a no-op —

- `ALTER TABLE … ADD COLUMN IF NOT EXISTS …`
- wrap `ADD CONSTRAINT` in `DO $$ BEGIN … EXCEPTION WHEN duplicate_object OR duplicate_table THEN null; END $$;`
  — a duplicate `UNIQUE`/PK constraint raises `duplicate_table` (42P07), a duplicate FK raises
  `duplicate_object` (42710), so catch both.

`0000_public_firedrake.sql` and `0001_aromatic_metal_master.sql` are the worked examples.

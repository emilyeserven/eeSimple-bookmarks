...
/** Association tab: pick which people are connected to this publisher. Auto-saves on toggle. */
export function PublisherPeopleForm({
  publisher,
}: Props) {
  const {
    data: people,
          ...
          <ul className="space-y-2">
            {(people ?? []).map(person => (
              ...
              >
                <Checkbox
                  id={`person-${person.id}`}
                  checked={person.publisherIds.includes(publisher.id)}
                  onCheckedChange={() =>
                    update.mutate(
                      {
                        id: person.id,
                        input: {
                          publisherIds: toggleId(person.publisherIds, publisher.id),
                        },
                      },
                      {
...
export function PublisherPeopleView({
  publisher,
}: Props) {
  const {
    data: people,
  } = usePeople();
  const connected = (people ?? []).filter(a => a.publisherIds.includes(publisher.id));

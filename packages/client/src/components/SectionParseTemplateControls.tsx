import type { ParseTag } from "@eesimple/types";
...
interface SaveParseTemplatePopoverProps {
  delineator: string;
  pattern: string;
  fallbackTag: ParseTag;
}

/** Popover with a name field to save the current delineator/pattern/fallback as a reusable template. */
export function SaveParseTemplatePopover({
  delineator, pattern, fallbackTag,
}: SaveParseTemplatePopoverProps) {
  const {
    t,
  } = useTranslation();
...
interface LoadParseTemplateDropdownProps {
  onLoad: (template: { delineator: string;
    pattern: string;
    fallbackTag: ParseTag; }) => void;
}

/** Popover listing saved parse templates; clicking one loads its delineator/pattern/fallback. */
export function LoadParseTemplateDropdown({
  onLoad,
}: LoadParseTemplateDropdownProps) {
  const {
    t,
  } = useTranslation();
  const [open, setOpen] = useState(false);

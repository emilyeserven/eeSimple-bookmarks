...
export function PanelTypeTiles() {
  const { openType } = usePanelControls();
  const [query, setQuery] = useState("");

  const needle = query.trim().toLowerCase();
  const filteredTypes = useMemo(
    () => (needle ? PANEL_CONTENT_TYPES.filter(({ label }) => label.toLowerCase().includes(needle)) : PANEL_CONTENT_TYPES),
    [needle],
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative flex items-center">
        <Search className="pointer-events-none absolute left-2.5 size-3.5 shrink-0 text-muted-foreground" />
        <Input
          type="text"
          value={query}

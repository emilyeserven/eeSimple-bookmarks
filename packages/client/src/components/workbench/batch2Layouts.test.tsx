...
describe("tag default layout", () => {
  ...
  it("renders the General tab's edit fields in the old TagGeneralForm order (#1193)", () => {
    ...
    expect(general?.sections).toEqual([{
      key: "general",
      fields: ["name", "primaryLanguage", "description", "names", "parent", "options", "genreMoods", "promote", "clearAssociations"],
    }]);
  });
});
...
describe("custom property default layout", () => {
  ...
  // The atomized General composite (#1196): one section, six fields ordered so each mode drops the
  // fields it lacks a renderer for — edit keeps name/type/status/description/aiInstructions; view
  // keeps status/description/aiInstructions/created (name/type are edit-only, created is view-only).
  it("splits the General tab into granular view fields", () => {
    const general = shape(propertyWorkbench, "view").find(tab => tab.key === "general");
    expect(general?.sections).toEqual([{
      key: "general",
      fields: ["status", "description", "aiInstructions", "created"],
    }]);
  });
  ...
  it("splits the General tab into granular edit fields", () => {
    ...
    expect(general?.sections).toEqual([{
      key: "general",
      fields: ["name", "type", "status", "description", "aiInstructions"],
    }]);
  });
});

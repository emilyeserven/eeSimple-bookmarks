...
/** Association tab: pick which YouTube channels are connected to this author. Auto-saves on toggle. */
export function AuthorYouTubeChannelsForm({
  author,
}: Props) {
  const {
    data: channels,
  ...
  const enabledIds = author.youtubeChannelIds;
...
export function AuthorYouTubeChannelsView({
  author,
}: Props) {
  const {
    data: channels,
  } = useYouTubeChannels();
  const connected = (channels ?? []).filter(ch => author.youtubeChannelIds.includes(ch.id));

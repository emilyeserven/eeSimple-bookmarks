          ...
          <ul className="space-y-2">
            {(authors ?? []).map(author => (
              ...
              >
                <Checkbox
                  id={`author-${author.id}`}
                  checked={author.websiteIds.includes(website.id)}
                  onCheckedChange={() =>
                    update.mutate(
                      {
                        id: author.id,
                        input: {
                          websiteIds: toggleId(author.websiteIds, website.id),
                        },
                      },
                      {
...
export function WebsiteAuthorsView({
  ...
  const connected = (authors ?? []).filter(a => a.websiteIds.includes(website.id));

 ...
 * toast is fired so the Notifications log records the real reason, not a generic string. `fallback`
 * is translated as its own phrase key here so every caller's fixed fallback text localizes without

...
async function startGateway() {
  ...
  if (existsSync(clientDist)) {
    ...
    // Only respond with the app shell when the request explicitly accepts text/html (i.e. a
    // browser navigation). Asset requests — JS, CSS, fonts, images — and the browser's service
    // worker update fetch all send `Accept: */*` without text/html, so missing assets (e.g. sw.js
    // or share-target-sw.js absent from the build) get a proper 404 rather than the HTML shell.
    // Serving HTML for a service worker script causes Chrome to report "An unknown error occurred
    // when fetching the script" because the MIME type is wrong for a SW registration/update.

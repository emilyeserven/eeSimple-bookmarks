/**
 * Pure URL canonicalization shared by the client (the bookmark-form cleanup + link preview) and the
 * middleware (newsletter ingest unwraps tracker links, then canonicalizes the result). It depends
 * only on the shared `Website` shapes, so it lives in `@eesimple/types` as the single source of
 * truth — don't re-implement tracker stripping or shortened-link expansion elsewhere.
 */

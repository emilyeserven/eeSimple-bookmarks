...
5. **Boot backfill for the existing config.** "Absent = hidden", so the stored config won't show the
   new field until backfilled. Add an idempotent backfill boot step in `services/cardDisplayRules.ts`
   (mirror `backfillCardDisplaySections`), wired into `src/index.ts` after `ensureCardDisplayConfig()`:
   inject the key into its default section for the stored config if it lacks it. The earlier reshape
   backfills were retired once applied (issue #862; the `db-schema-change` skill's "Pruning spent
   steps"), so there's no sibling to copy anymore — model it on the empty-array note in `migrate.ts` and
   prune your step the same way once it has run in production.
